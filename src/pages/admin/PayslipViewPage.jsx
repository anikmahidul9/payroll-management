...
const PayslipViewPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [employeeDoc, deductionsSnapshot] = await Promise.all([
  ...
  const calculatePayroll = () => {
    if (!employee) return { earnings: [], deductionsDetail: [], grossSalary: 0, totalDeductions: 0, netSalary: 0 };

    const grossSalary = employee.baseSalary || 0;
    const earnings = [{ name: 'Base Salary', amount: grossSalary }];
    
    const deductionsDetail = deductions.map(deduction => {
      let amount = 0;
    ...
    const totalDeductions = deductionsDetail.reduce((acc, curr) => acc + curr.amount, 0);
    const netSalary = grossSalary - totalDeductions;

    return { earnings, deductionsDetail, grossSalary, totalDeductions, netSalary };
  };

  const handleDownloadPdf = () => {
    const input = payslipRef.current;
    
    // CSS override to replace modern color functions with simple hex/rgb codes
    const cssOverride = `
        <style>
    ...
    html2canvas(input, {
        ...
        pdf.save(`Payslip-${employee?.name}-${new Date().toLocaleDateString()}.pdf`);
    }).catch(err => {
  ...
  if (!employee) return <div className="flex justify-center items-center min-h-screen">No Employee Data Found.</div>;

  const { earnings, deductionsDetail, grossSalary, totalDeductions, netSalary } = calculatePayroll();
  const payslipDate = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        ...
        <div ref={payslipRef} className="bg-white shadow-lg rounded-lg p-8 md:p-12">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              {deductionsDetail.map((item, index) => (
                <div key={index} className="flex justify-between items-center mb-2">
                  <span className="text-gray-700">{item.name}</span>
                  <span className="text-gray-800 font-medium">-${item.amount.toFixed(2)}</span>
                </div>
          ...
          <div className="border-t-2 border-gray-200 pt-6">
            <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                    ...
                    <p className="text-2xl font-bold text-red-500">-${totalDeductions.toFixed(2)}</p>
                </div>
                <div>
                    <p className="text-gray-500 uppercase tracking-wider text-sm">Net Salary</p>
